...
class BaseReport(object):
    ...
    )  # all instance methods that inherit from this class will be admin required
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        for metric in metrics.keys():
            for _date in context["date_list"]:  # going through all the dates
                if (
                    metrics[metric].get(getattr(_date, self.date_grouping_attr), None) is None
                ):  # fill in empty ones
        ...
        context["date_list"] = [
            d.strftime(getattr(self, "format_string")) for d in context["date_list"]
        ]
...
class MonthlyMetricReport(BaseMetricReport, MonthArchiveView):
    ...
    def create_date_list(self, date_list):
        ...
        return list(
            BaseMetricReport.create_date_range(
                DAILY,
                datetime.date(found_day.year, found_day.month, 1),
...
class YearlyMetricReport(BaseMetricReport, YearArchiveView):
    ...
    def create_date_list(self, date_list):
        ...
        return list(
            BaseMetricReport.create_date_range(
                MONTHLY, datetime.date(found_day.year, 1, 1), datetime.date(found_day.year, 12, 1)
            )
        )
